...
function amcharts_shortcode ( $atts ) {
  ...
  foreach ( $libs as $lib ) {
    wp_enqueue_script( 'amcharts-external-' . md5( $lib ), $lib, array(), AMCHARTS_VERSION, true );
  }
...
function amcharts_enqueue_scripts() {
  ...
  //wp_enqueue_script( 'codemirror-htmlmixed', plugins_url( 'lib/codemirror/mode/htmlmixed/htmlmixed.js', AMCHARTS_BASE ), array( 'htmlmixed' ), AMCHARTS_VERSION );
}
